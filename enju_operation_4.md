...
<tr>
	...
	<td>半角数字のみ, システムが自動付与するID, 「資料の表示」画面のURL末尾にある数値。更新や削除に使用します。※新規作成時には使用しないでください。<!-- ( #872 ) いずれ：新規作成時にはこの列は無視されます。-->　※Version 1.1.0では、削除には使用できません。<!-- ( #805 ) --></td>
	<td>4</td>
</tr>
<tr>
...
#### 1.「[4-8 書誌と所蔵のエクスポートをする](enju_operation_4.html#section4-8)」を実行します。
