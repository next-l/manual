...
検索時に異体字を同一視させるためには、Solrにそのような設定を行わなければなりません。本章ではその手順を説明します。
...
これを利用して、①確かな典拠に基づくメインファイルと、②館の都合に合わせて追加／修正するカスタマイズファイル、といったようにファイルを分割管理することができます。
...
このままでは設定が反映されないので、以下のようにして Enju を再起動（リスタート）します。

	sudo restart enju_leaf

これで異体字でも検索ができるようになるはずです。

古い仮想マシン場合、このやり方で再起動しないかもしれません。インストールマニュアル「第7章 Enjuのアップデート」をご確認ください。
...
このページにCharFilterの仕様などが詳しく書かれています。公式サイトです。
...
このように、ある二つの漢字が異体字であるかどうかはそれが使われた時代によって変ります。
