...
システムの標準設定を変更する必要がでることは基本的にはありません。
[人物・団体の新規作成や編集](enju_operation_4.html#section4-9)の際に表示される
...
（注： Enju Leaf 1.1.0では、削除してはいけない種類にも，
削除リンクが表示され削除できてしまうバグがありますので，
基本的に削除は行わないでください。）

{::comment}
ただし、この種類を使った「人物・団体」があるものについては[削除]リンクは
表示されず、削除できません。 （#781が解決したらこちらに差し替え）{:/comment}
