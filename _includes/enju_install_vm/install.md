...
![ブラウザで起動の確認](assets/images/image_install_078_4.png)
	...
	【Column】[3-1-3 IPアドレスの確認](enju_install_vm_3.html#3-1-3) の 6. で確認した「IPv4のアドレス」（例：```192.168.11.9```）でも確認できます。
	その場合は```http:://IPv4のアドレス:8080```（例：```http://192.168.11.9:8080```）と入力します。
	</div>
